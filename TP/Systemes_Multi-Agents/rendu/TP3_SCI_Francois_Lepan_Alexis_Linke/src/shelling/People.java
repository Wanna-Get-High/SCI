	...
	public void doAction() {
		...
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				...
				// if in the environement and there is someone
				if(newX >= 0 && newY >= 0 &&
					newX < size && newY < size &&
					agents[newX][newY]!=null) {
					
					nNeighbor++;
					// increment satisfaction if he has neighbor of the same kind around him
		...
		this.satisfaction = nNeighbor;
		
		// if he has neighbor then set the satisfaction 
